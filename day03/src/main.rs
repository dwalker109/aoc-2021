...
fn part_1(input: &'static str) -> u32 {
    ...
    let bit_pop = (0..bitness).map(|pos| get_bit_popularity(&bits, pos)).collect::<Vec<_>>();
...
fn parse_input(input: &str) -> Vec<Vec<char>> {
    input.lines().map(|line| line.chars().collect::<Vec<_>>()).filter(|c| c.len() > 0).collect()
}
...
fn get_bit_popularity(input: &[Vec<char>], pos: usize) -> Vec<(char, usize)> {
    ...
    counts.into_iter().sorted_by(|a, b| Ord::cmp(&a.1, &b.1)).collect_vec()
}

fn find_via_bit_criteria(bitness:usize, bits: &Vec<Vec<char>>, winner: usize) -> Vec<char> {
    let default = char::from_digit(winner as u32, 2).unwrap();

    (0..bitness).fold_while(bits.clone(), |rem_bits, pos| {
        let bit_pop = get_bit_popularity(&rem_bits, pos);
        let to_keep = match bit_pop[0].1 == bit_pop[1].1{
            true => default,
            false => bit_pop[winner].0,
        };
        let rem = rem_bits.into_iter().filter(|curr| curr[pos] == to_keep).collect_vec();
    ...
    }).into_inner().into_iter().flatten().collect_vec()
}
